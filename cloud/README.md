...
flowchart LR
  %% Edge & Routing
  Users[Users (Web/Mobile)] --> R53[Amazon Route 53]
  R53 --> CF[Amazon CloudFront]
  CF -->|TLS| ALB[(Application Load Balancer)]
  ...
  subgraph VPC[VPC]
    ...
    subgraph Private[Private Subnets]
      direction LR
      UTask[ECS Fargate: User Service]
      PTask[ECS Fargate: Payment Service]
      NTask[ECS Fargate: Notification Service]
      ...
      %% ALB to services (path-based)
      ALB -->|/users*| UTask
      ALB -->|/payments*| PTask
      ALB -->|/notifications*| NTask
  ...
  CC --> CPL --> CBL --> ECR
  ECR -- pull image --> UTask
  ECR -- pull image --> PTask
  ECR -- pull image --> NTask
